# AirPlay Service 开发记录（2026-07-14）

本文记录 `airplay_service` 第一阶段的实现、真机调优结论和后续图形 App 接入注意事项。生产版本为 `0.3.0`，目标为 HoloCubic ESP32-S3、ESP-IDF 6.0.2 和 AirPlay 1/RAOP 音频。
...
2. 原生模块把 RTP 放入 128 包 PSRAM 抖动缓冲，预缓冲 48 包。
3. core 1 的生产者完成 AES-CBC、ALAC 和电平计算，把 PCM 放入 32 包队列。
4. core 0 的消费者只负责阻塞式 I2S 写入，避免“解码耗时 + 8 ms 播放耗时”串行相加。
5. 零星缺包最多等待 12 个周期；PCM 队列低于 24 包时取消等待并单帧掩蔽，防止长阻塞形成级联欠载。
6. RTP 缓冲达到高水位时跳到最新安全窗口，作为异常网络情况下的最后保护。

关键默认值：

| 参数 | 值 | 目的 |
| --- | ---: | --- |
| RTP slots | 128 | 乱序、重传和网络抖动 |
| RTP prebuffer | 48 包（约 384 ms） | 启播和 Wi-Fi 抖动余量 |
| PCM slots | 32 包（约 256 ms） | 解码/I2S 解耦 |
| retransmit floor | 24 包 | 保留约 192 ms 播放安全余量 |
| I2S DMA | 12×512 | 减少短时调度抖动 |
| producer / output core | 1 / 0 | 双核流水线 |
| producer / output priority | 8 / 9 | I2S 输出优先 |
